...
- ⏳ **Fase 5 (parte 2b, pendiente)** — merge `gastos`+`gastos_vehiculos` → `facturas_recibidas(_lineas)` con imputación por vehículo/viaje; cablear `series` en la numeración de facturas; uso de `inmovilizado`/`amortizaciones`/`vencimientos`. Cambios de código (núcleo contable) + tests de integración.
