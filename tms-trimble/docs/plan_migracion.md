...
- ✅ **Fase 5 (parte 2b, UNIFICACIÓN TOTAL — revisada)** — `gastos`/`gastos_vehiculos` son **tablas** (única fuente de verdad) y `finanzas.facturas_recibidas(_lineas)` es la capa analítica, alimentada por triggers **AFTER INSERT/UPDATE/DELETE** (`gastos_sync_fr`/`gastos_veh_sync_fr`). La sincronización la garantiza la BD, no el código: los 4 flujos Python escriben SOLO en `gastos`/`gastos_vehiculos`, y el trigger replica a `facturas_recibidas` vía `gasto_origen`. Vistas públicas auto-updatable (`SELECT * FROM finanzas.*`), endpoints sin cambios. Conciliación diaria (debe dar 0 filas) en `docs/conciliacion_gastos.sql` + cron 7:00. Verificado: insertar/editar/borrar/pagar gasto → factura reflejada; backfill 0 pendientes; conciliación 0 filas en producción.
...
- ✅ **Fase 9 (limpieza)** — código migrado a nombres cualificados por esquema (`finanzas.*`, `flota.*`, `operaciones.*`, `sistema.*`) en ~440 referencias; **20 vistas alias puras ELIMINADAS** (gastos, gastos_vehiculos, costes_fijos, liquidaciones, cuentas, asientos, apuntes, facturas, factura_lineas, mantenimientos, paradas, tramos, sync_state, audit_log, config, config.roles, integracion_proveedores/campos/valores, actividades). Se **mantienen** 8 abstracciones legítimas (no alias): `clientes`/`proveedores`/`transportistas` (filtro NIF sobre terceros), `conductores` (JOIN rrhh+empleados), `config.usuarios` (JOIN que mapea `rol` nombre→`rol_id`), `vehiculos`/`trips` (remap PK: `id`=codigo), `finanzas.rentabilidad_viaje`. Eliminado el `CREATE TABLE IF NOT EXISTS audit_log` legacy del bloque de migración. Verificado: 0 vistas alias restantes, 27 tests, 16 endpoints 200, trigger gastos→facturas_recibidas OK, conciliación 0 filas.
- ✅ **Fase 9 (telemetría)** — hypertable muerta `telemetria` (0 filas) ELIMINADA; `GET /api/telemetria` ahora lee `telemetria.posiciones_gps` (historial real, 186k posiciones) con mapeo `fuente→source`, `speed_kmh→speed`, `odometer_km→mileage` (misma unidad). Verificado en producción: historial devuelve datos reales.
- ✅ **Fase 9 (cierre)** — la migración estructural queda **COMPLETA (Fases 0–9)**. Las únicas vistas que subsisten son abstracciones legítimas de negocio (clientes/proveedores/transportistas/conductores/config.usuarios/vehiculos/trips/rentabilidad_viaje), no alias de relocación; el resto del código opera ya contra nombres cualificados por esquema.
...
- ✅ **Fase 6/7 (remap PK)** — `vehiculos.id` y `trips.id` (TEXT = referencia Trimble) → **`id` BIGINT surrogate identity PK** + `codigo` TEXT UNIQUE (= antigua referencia) + `vehiculos.terminal_trimble` UNIQUE (= referencia SOAP). FKs `paradas/tramos→trips.codigo` reañadidas. Vistas compat exponen `id = codigo` (TEXT) para que código+worker+frontend sigan funcionando sin cambios; la surrogate queda oculta. Ejecutado con worker PARADO + backup previo. Verificado en producción: 50 vehículos + 2 viajes intactos, telemetría en vivo, API devuelve códigos correctos, upsert por `codigo` OK. Migración en `migracion_fase67_remap_pk.sql`.
