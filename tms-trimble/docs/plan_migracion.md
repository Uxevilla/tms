...
- ⏳ **Fase 5 (parte 2b, merge de código)** — merge `gastos`+`gastos_vehiculos` → `facturas_recibidas(_lineas)` con imputación por vehículo/viaje (reescribir el flujo de gastos + OCR + mantenimiento + subcontrata para escribir en las tablas nuevas); uso de `inmovilizado`/`amortizaciones`/`vencimientos`. Cambios de código (núcleo contable) + tests de integración.
