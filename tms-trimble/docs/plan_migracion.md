...
- ✅ **Fase 5 (parte 2b, UNIFICACIÓN TOTAL)** — `gastos` y `gastos_vehiculos` dejan de ser tablas: ahora son **vistas** que reconstruyen desde `finanzas.facturas_recibidas(_lineas)` (con `terminal`/`categoria`/`storage_key` preservados), con triggers INSTEAD OF de INSERT/UPDATE/DELETE que escriben en la cabecera+líneas. Tablas planas `finanzas.gastos`/`finanzas.gastos_vehiculos` ELIMINADAS; el dual-write + helper `_sync_factura_recibida` + columna `gasto_origen` eliminados. Código y frontend SIN cambios (operan vía las vistas). Verificado en producción: crear/editar/borrar gasto → `facturas_recibidas`+línea, tablas planas 0 restantes, health 200, telemetría en vivo.
