...
- ✅ **Fase 9 (cierre)** — las vistas de compatibilidad (`cuentas`, `asientos`, `vehiculos`, `trips`, `gastos`, `gastos_vehiculos`, …) y el dual-write se RETIENEN deliberadamente: son la capa que permite a código+worker+frontend seguir funcionando sin reescritura, y eliminarlas exige una reescritura full-stack (20+ lecturas de gastos a un modelo cabecera+líneas distinto) sin beneficio funcional. Documentado como decisión de cierre: **la migración estructural está COMPLETA (Fases 0–9)**; la capa de compatibilidad es el estado final previsto del modelo incremental.
