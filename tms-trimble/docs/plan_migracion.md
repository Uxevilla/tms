...
- ⏳ **Fase 5 (parte 2, pendiente)** — relocación de `cuentas/asientos/apuntes/facturas/gastos/…` al esquema `finanzas` + vistas; merge `gastos`+`gastos_vehiculos` → `facturas_recibidas`; cablear `series` en la numeración; `inmovilizado`/`amortizaciones`/`vencimientos`. Requiere cambios de código + tests de integración contra BD real (Claude).
