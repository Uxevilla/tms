...
def test_fail_closed_sin_password(monkeypatch):
    monkeypatch.setattr(main, "TRANSFOLLOW_WEBHOOK_PASSWORD", "")
    monkeypatch.setattr(main, "TRANSFOLLOW_WEBHOOK_USER", "transfollow")
...
def test_basic_auth_correcto(monkeypatch):
    monkeypatch.setattr(main, "TRANSFOLLOW_WEBHOOK_PASSWORD", "secreto")
    monkeypatch.setattr(main, "TRANSFOLLOW_WEBHOOK_USER", "transfollow")
