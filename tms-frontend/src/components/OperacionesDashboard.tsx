...
export function OperacionesDashboard() {
  ...
  const columnDefs = useMemo<ColDef<Viaje>[]>(
    () => [
      ...
      { field: "matricula", headerName: "Matrícula", width: 120, editable: true, cellClass: editableCell, cellEditor: "agSelectCellEditor", cellEditorParams: { values: Array.from(new Set(vehiculos.map((v) => v.matricula).filter(Boolean))) }, cellRenderer: panelCell("vehiculo", undefined, { icono: true }) },
      { field: "conductor", headerName: "Conductor", width: 170, editable: true, cellClass: editableCell, cellEditor: "agSelectCellEditor", cellEditorParams: { values: Array.from(new Set(conductores.map((c) => c.nombre).filter(Boolean))) }, cellRenderer: panelCell<Viaje>("conductor", (d) => conductores.find((c) => c.nombre === d.conductor)?.id, { icono: true }) },
